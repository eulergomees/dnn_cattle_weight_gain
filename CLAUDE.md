...
- ✅ **DNN** (`dnn_gmd.ipynb`) reexecutada nos **245 animais / 3 fazendas** (9 preditores — `proporcao_bos_indicus_pct` virou variável com as raças da Humberto): grid search → melhor **(128,64,32), dropout 0, lr 1e-2, BN=True, wd 0**. **CV 10-fold: R² 0,805, MAE 0,030** (baseline MAE 0,079). Subiu de 0,665 (155) — mais dados levantou o teto; parte do ganho de R² é o alvo ter mais variância (MAE 0,032→0,030 é o comparável). Jitter feature-only não ajuda. **LOPO melhorou:** testa Humberto R²+0,12, Elvis −0,12, mas **Sonico −154** (outlier: nascidos leves/ciclos longos/sal mineral).
