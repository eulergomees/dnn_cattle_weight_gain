...
**Última atualização:** 2026-09-22 (baselines)
...
- ✅ **Baselines feitos** (`baselines/01_linear.ipynb`, `02_xgboost.ipynb`) no `get_cv()` atual (5×49 por fazenda) — comparáveis com a DNN. `results/model_comparison.csv`: **DNN R² 0,820 (MAE 0,028) ≫ XGBoost 0,508 (MAE 0,051) > Linear 0,426 (MAE 0,063)**. **LOPO inverte:** XGBoost é o mais equilibrado (Elvis −0,74 / Humberto 0,29), DNN quebra mais (Elvis −49,3 / Humberto 0,35), Linear é o pior nos dois (Elvis −70,96 / Humberto −1,61). Testar em **Elvis** é sistematicamente pior que testar em Humberto nos 3 modelos — efeito do dataset (Elvis ficou maior/mais heterogêneo após a fusão do Sonico), não só da DNN.
- ⏸ **TabPFN pendente:** `baselines/03_tabpfn.ipynb` pronto, mas `tabpfn` exige licença aceita manualmente (sem terminal interativo aqui) — https://ux.priorlabs.ai, aceitar na aba "Licenses", copiar a API key e setar `TABPFN_TOKEN` antes de rodar.
- **Próximo:** rodar TabPFN assim que a licença for aceita; depois **CV agrupado por lote** (cohort = propriedade+data_entrada+data_saida, testa se o R² da DNN é inflado por vazamento) e **SHAP** no melhor modelo (cuidado com a multicolinearidade `media_suplemento`×`pb_suplemento`, r=0,93).
